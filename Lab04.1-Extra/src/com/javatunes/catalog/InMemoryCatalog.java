...
public class InMemoryCatalog implements Catalog {
    ...
    private static List<MusicItem> catalogData = new ArrayList<>(List.of(
    ...
    public Collection<MusicItem> findByCategory(MusicCategory category) {
        ...
        for(MusicItem item : catalogData) {
            if (item.getMusicCategory() == category) {  // == is OK for enum types
                result.add(item);
            }
        }
        return result;
     ...
     */
    public static int genreCount(MusicCategory category) {
        int count = 0;
        for (MusicItem item : catalogData) {
